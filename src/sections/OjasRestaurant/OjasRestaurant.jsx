...
const culinaryImages = [
  {
    src: ojasDishTwo,
    title: "Paneer Methi Malai",
    eyebrow: "Signature comfort",
    alt: "Paneer methi malai Ojas Veg Restaurant premium food presentation.",
    className: "",
  },
  {
    src: ojasDishThree,
    title: "Creamed Paneer",
    eyebrow: "Slow kitchen warmth",
    alt: "Ojas Veg Restaurant paneer dish served in a pan with warm cinematic styling.",
    className: "",
  },
  {
    src: ojasDishOne,
    title: "Bhogichi Thali",
    eyebrow: "Regional plate",
    alt: "Ojas Veg Restaurant thali with assorted breads, rice, dal, and vegetarian accompaniments.",
    className: "",
  },
];
...
function CulinaryImageCard({ image, index }) {
  ...
  return (
    <motion.figure
      className={cx(
        "group relative overflow-hidden rounded-lg border border-ivory-50/10 bg-[linear-gradient(150deg,rgba(255,250,240,0.07),rgba(216,183,88,0.018)),rgba(7,5,4,0.72)] p-2 shadow-[0_30px_88px_rgba(0,0,0,0.38)]",
        image.className,
      )}
      variants={fadeUp}
      whileHover={{ y: -8 }}
    ...
    >
      ...
      <div className="relative aspect-[4/3] overflow-hidden rounded-md bg-ink-950">
        ...
        />
        <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(180deg,rgba(5,4,3,0.04),transparent_38%,rgba(5,4,3,0.78)),radial-gradient(circle_at_50%_18%,rgba(255,250,240,0.1),transparent_15rem)]" />
        <figcaption className="absolute inset-x-0 bottom-0 p-5 sm:p-6">
          <p className="text-[0.68rem] font-bold uppercase tracking-[0.2em] text-gold-100/78">{image.eyebrow}</p>
          <p className="mt-2 font-display text-3xl font-semibold leading-none text-ivory-50 sm:text-[2rem]">
            {image.title}
          </p>
          <p className="mt-3 h-px w-16 bg-gradient-to-r from-gold-200/72 to-transparent" />
        </figcaption>
      </div>
    ...
    >
      ...
      <motion.div className="mt-8 grid gap-4 sm:grid-cols-3 sm:items-stretch lg:mt-10 lg:gap-5" variants={staggerContainer}>
        {culinaryImages.map((image, index) => (
          <CulinaryImageCard key={image.title} image={image} index={index} />
        ))}
      </motion.div>
